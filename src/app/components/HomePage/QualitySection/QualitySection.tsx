  ...
  const qualities = [
    {
      ...
      description:
        "Lynk LMS ensures the best user experience with instant support.",
    },
    {
      ...
      description: (
        <>
          ...
          <a
            href="mailto:support@lynklms.com"
            className="underline decoration-[1px] hover:text-gray-200"
          >
            support@lynklms.com
          </a>
          .
    ...
    },
    {
      ...
      description:
        "Enjoy 99.99% uptime and scalable products for uninterrupted service.",
    },
  ];
  const qualityPoints = [
    {
    ...
    <div className="xl:py-[100px] sm:py-[72px] py-[42px] flex flex-col xl:gap-[92px] sm:gap-[52px] gap-[46px] xl:px-0 sm:px-4 justify-center items-center">
      <div className="flex flex-col xl:px-0 px-4 max-w-6xl 3xl:min-w-[1590px] xl:gap-[68px] sm:gap-[38px] gap-7 items-center">
        <div className="flex flex-col xl:gap-4 sm:gap-[10px] gap-2 items-center">
          <p className="font-bold 3xl:text-[48px] 3xl:leading-10 xl:text-[36px] xl:leading-[40px] sm:text-[32px] sm:leading-[43.71px] text-[24px] leading-[32.8px] text-[#1D1A27]">
            Fast. Reliable. Effective.
          </p>
          <p className="font-normal 3xl:w-[1060px] 3xl:text-[20px] 3xl:leading-[27.32px] xl:text-[16px] xl:leading-[21.86px] text-[14px] leading-[19.12px] text-[#1D1A27CC] xl:max-w-full sm:max-w-[612px] max-w-[343px] text-center">
            With 24/7 customer support, robust quality assurance, and a
            user-friendly interface, we ensure a hassle-free and effective
            learning experience.{" "}
          </p>
        </div>
        <div className="grid sm:grid-cols-3 grid-cols-2 justify-between 3xl:gap-[32px] xl:gap-[15px] 3xl:min-w-[1590px] w-full gap-[12px]">
          {qualities?.map((quality, index) => {
            return (
              <div
                key={index}
                className={`3xl:rounded-[24px] xl:rounded-[42px] rounded-[25px] 3xl:py-[40px] xl:py-10 py-6 3xl:px-[42px] xl:px-[34px] px-5 bg-[#635BFF] flex flex-col 3xl:gap-[8px] xl:gap-6 gap-3 w-full ${
              ...
              >
                <div className="flex flex-col 3xl:gap-[16px] xl:gap-7 gap-4">
                  ...
                  <p className="font-bold 3xl:text-[24px] 3xl:leading-8 xl:text-[18px] xl:leading-6 text-[16px] leading-[14px] text-[#FFFFFF]">
                    {quality?.title}
                  </p>
                </div>
                <p className="font-normal 3xl:text-[22px] 3xl:leading-8 xl:text-[16px] xl:leading-6 sm:text-[14px] sm:leading-[19.12px] text-[13px] leading-[17.76px] text-[#FFFFFF]">
                  {quality?.description}
                </p>
              </div>
            );
            ...
            <div className="flex flex-col gap-4">
              {qualityPoints?.map((quality, index) => {
                return (
                  <div
                    key={index}
                    className="flex items-start xl:gap-[5px] gap-[3px]"
                  >
                    ...
                    <p className="font-normal 3xl:text-[20px] 3xl:leading-8 xl:text-[16px] xl:leading-6 text-[14px] leading-[20px] text-[#1D1A27CC]">
                      {quality?.text}
                    </p>
                  </div>
                );
