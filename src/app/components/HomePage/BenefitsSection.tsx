...
const BenefitsOverviewSection = () => {
    ...
    const features = [
        ...
        { title: "Platform Flexibility", lynk: `Complete freedom`, saas: "Restricted to provider's ecosystem" },
    ];
    return (
        <div
            id="intuitive-lms"
                ...
                <div className="md:w-full md:flex md:justify-center md:items-center w-full">
                    ...
                    >
                        ...
                        <div className="flex flex-col flex-1 rounded-tl-[16px] rounded-bl-[16px] overflow-hidden " >
                            ...
                            {/* Features list - only 6 rows with bg-white */}
                            {features.map((feature, idx) => (
                                <div
                                    key={idx}
                                    className={`h-[78px] flex items-center px-[10px] xl:px-[48px] lg:px-[30px] sm:px-[10px] 3xl:text-[16px] xl:text-[14px] font-[600] text-[#1D1A27] bg-white ${idx !== features.length - 1
                                            ? 'border-b border-[#6B728080]'
                                            : 'rounded-bl-[16px]'
                                        }`}
                                    style={idx === features.length - 1 ? { boxShadow: "0px 0px 50px 0px #6459FF17" } : {}}
                                >
                                    {feature.title}
                                </div>
                            ))}
                            {/* Empty space with transparent background and only bottom shadow */}
                            <div className="h-[84px] bg-transparent relative" style={{
                                boxShadow: "0px 25px 50px 0px #6459FF17", // ONLY bottom shadow
                                zIndex: -1
                            }}></div>
                        </div>
                        {/* Lynk Column */}
                            ...
                            <div className="bg-[#F7F7FF] flex flex-col justify-between rounded-[14px] h-full">
                                ...
                                <div className="flex-grow">
                                    {features.map((feature, idx) => (
                                        <div
                                            key={idx}
                                            className={`h-[78px] flex items-center px-[10px] xl:px-[48px] lg:px-[30px] sm:px-[10px] 3xl:text-[16px] xl:text-[14px] font-[600] text-[#1D1A27] ${idx !== features.length - 1 ? 'border-b border-[#6B728080]' : ''
                                                }`}
                                        >
                                ...
                                {/* Button Row - Only in the Lynk column */}
                        ...
                        <div className="flex flex-col flex-1 rounded-tr-[16px] rounded-br-[16px] overflow-hidden">
                            ...
                            {/* Features - only 6 rows with bg-white */}
                            {features.map((feature, idx) => (
                                <div
                                    key={idx}
                                    className={`h-[78px] flex items-center px-[10px] xl:px-[48px] lg:px-[30px] sm:px-[10px] 3xl:text-[16px] xl:text-[14px] font-[600] text-[#1D1A27] bg-white ${idx !== features.length - 1
                            ...
                            {/* Empty space with transparent background and only bottom shadow */}
                            <div className="h-[84px] bg-[#FAFAFA] relative" style={{
                                boxShadow: "0px 25px 50px 0px #6459FF17", // ONLY bottom shadow
                                zIndex: -1
                            }}></div>
                        </div>
