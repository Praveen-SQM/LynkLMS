      ...
      <div className="grid sm:grid-cols-3 max-w-6xl 3xl:min-w-[1590px] grid-cols-2 xl:gap-6 sm:gap-3 gap-2 w-full px-4 xl:px-0 ">
        {lmsFeatures?.map((feature, index) => {
          return (
            <div
              key={index}
              className={`flex flex-col sm:rounded-[20px] transition-colors rounded-[12px] xl:p-10 p-4 xl:gap-5 gap-2 bg-[#ffffff] border-2 border-transparent hover:border-red-500 hover:border-2 ${index===lmsFeatures?.length-1 ? "sm:col-span-1 col-span-2" : ""}`}
            >
